...
# Рабочая таймзона для дат в таблице
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not re.search(r"Маркет\.Операции|Яндекс\.Маркет", text, re.IGNORECASE):
        return

    # Telegram отдаёт message.date в UTC.
    # Здесь переводим дату сообщения в московское время.
    msg_date = message.date
    local_msg_date = msg_date.astimezone(LOCAL_TZ)

    contractor = parse_contractor(text)
    period_end = parse_period_end_date(text, local_msg_date)
    ...
    row = {
        "date": local_msg_date.strftime("%Y-%m-%d"),
        "forecast": forecast_date(contractor, period_end, local_msg_date),
...
def main():
    if not BOT_TOKEN:
        raise RuntimeError("BOT_TOKEN не найден. Проверь переменную BOT_TOKEN.")
